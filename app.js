...
const API_URL = "https://script.google.com/macros/s/AKfycbyKjKKdUsfew2mX3nQ5Yr8kdWxyiIPAjd-GrCuR4K8/exec";
...
function callApi(action, payload = {}) {
  ...
  return new Promise(
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error("Koneksi ke server terlalu lama. Periksa internet dan URL Apps Script."));
    }, 30000);

    function handler(event){
      if(event.source !== $("apiBridge").contentWindow) return;
      const msg = event.data || {};
      if(msg.type !== "MTC_RESPONSE" || msg.requestId !== requestId) return;

      clearTimeout(timer);
      window.removeEventListener("message", handler);

      if(msg.success) resolve(msg.data);
      else reject(new Error(msg.error || "Terjadi kesalahan pada server."));
    }

    window.addEventListener("message", handler);

    $("apiBridge").contentWindow.postMessage({
      type: "MTC_REQUEST",
      requestId,
      action,
      payload
